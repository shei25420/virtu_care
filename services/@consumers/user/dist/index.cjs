...
const RabbitMQ__default = /*#__PURE__*/_interopDefaultCompat(RabbitMQ);
const crypto__default = /*#__PURE__*/_interopDefaultCompat(crypto);

const rabbitmq = fp__default(async (fastify) => {
  try {
    const rmq = new RabbitMQ__default(process.env.RABBITMQ_URL || "amqps://gztpajlb:v2sZ-56M_GFyI6fbDY7LEc3LfFZmKFoN@shrimp.rmq.cloudamqp.com/gztpajlb");
    ...
    register: async (req, reply) => {
      try {
        const responsePromise = new Promise(async (resolve, reject) => {
          ...
          const correlationId = crypto__default.randomBytes(16).toString("hex");
